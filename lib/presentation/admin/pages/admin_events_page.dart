...
class _AdminEventsPageState extends State<AdminEventsPage> {
  ...
  // ✅ API Realtime avec dynamic pour éviter les problèmes de typage
    ...
    try {
      ...
      (_channel as dynamic)
          ...
          .subscribe((status, [error]) {
            debugPrint('AdminEventsPage realtime: status=$status, error=$error');
          });
    } catch (e) {
              ...
              children: [
                Row(
                  children: [
                    ...
                    if (isFeatured) _TagBadge(label: 'Premium', icon: Icons.stars_rounded, color: Colors.amber),
                    if (soldOut) const SizedBox(width: 8),
                    if (soldOut) _TagBadge(label: 'SOLD OUT', icon: Icons.block_rounded, color: AdminCyberColors.danger),
                  ],
                ),
                const SizedBox(height: 4),
                Text(
          ...
          child: Container(
            ...
            child: Column(
              ...
              children: [
                ...
                _TwoCol(
                  wide: wide,
                  left: _DateTimeField(value: _startsAt, enabled: !_saving, onChanged: (v) => setState(() => _startsAt = v)),
  ...
  Future<void> _save() async {
    ...
    try {
      final eventId = await widget.service.upsertEvent(
        ...
        speakers: _speakers.text.split('\n').map((e) => e.trim()).where((e) => e.isNotEmpty).map((name) => {'name': name}).toList(growable: false),
        sponsors: _sponsors.text.split('\n').map((e) => e.trim()).where((e) => e.isNotEmpty).map((name) => {'name': name}).toList(growable: false),
        agenda: _agenda.text.split('\n').map((e) => e.trim()).where((e) => e.isNotEmpty).map((line) {
      ...
      child: Column(
        ...
        children: [
          ...
          SegmentedButton<String>(
            ...
            onSelectionChanged: onChanged == null ? null : (sel) {
