...
class _UserEventDashboardPageState extends State<UserEventDashboardPage> {
  ...
  void initState() {
    ...
    // ✅ Correction : constructeur positionnel (sans nom de paramètre)
  ...
  Future<void> _loadTickets() async {
    try {
      ...
      final registrations = await _eventService.getUserRegistrations(
        user.id,
      );
  ...
  Future<void> _cancelTicket(EventRegistration registration) async {
    await _eventService.cancelRegistration(registration.id);
    await _loadTickets();
                  ...
                  : ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return _TicketCard(
                          ...
                          event: event,
                          onViewTicket: () {
                            context.push('/events/${event.id}/ticket/${registration.id}');
                          },
                          onCancel: () {
                            _showCancelDialog(registration);
                          },
                        );
                      },
                    ),
  ...
  void _showCancelDialog(EventRegistration registration) {
    showDialog(
      context: context,
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              await _cancelTicket(registration);
            },
